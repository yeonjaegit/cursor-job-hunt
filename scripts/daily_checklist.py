...
**Note**: 학습은 월/화/수/금 주 4일 진행합니다. (수요일은 유동)
"""
    ...
    # 목요일 (운동만)
    if weekday_num == 3:
        content = f"""# 📋 {date_str} ({weekday}) - 운동만
...
"""
        checklist_path.write_text(content, encoding="utf-8")
        print(f"✅ 운동만 체크리스트 생성: {checklist_path}")
        return checklist_path
    
    # 수요일 (유동 학습일)
    if weekday_num == 2:
        content = f"""# 📋 {date_str} ({weekday}) - 유동 학습일

## 📚 오늘은 유동 학습일!

09:00부터 시작하지만 끝나는 시간은 자유롭게 조정하세요.
...
**Note**: 운동 없음. "오늘은 복습까진 못할거같아" 등으로 상황 전달 후 기록하세요.
"""
        checklist_path.write_text(content, encoding="utf-8")
        print(f"✅ 유동 학습일 체크리스트 생성: {checklist_path}")
        return checklist_path
...
### 1. 코딩테스트 (목표: 최대 2문제, 2시간 학습)
- [ ] 학습 시간: 2시간 완료
- [ ] 완료한 문제 수: ___개
- [ ] 학습 내용 (0문제여도 OK):
  - 문제 1: ____________ (완료/시도/개념학습)
  - 문제 2: ____________ (완료/시도/개념학습)
  - 복습한 개념: ____________
